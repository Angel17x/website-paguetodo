...
function translate(value){
  switch(value.toLowerCase()){
    case 'pos':
					return 'POS'
				case 'payment_solutions':
					return 'Soluciones de pago'
        case 'service_pay':
					return 'Recarga'
				case 'cashing':
					return 'Cobranza'
        ...
        case 'collection':
          return 'Recaudación'
        case 'MOVILNET_PREPAY':
          return "MOVILNET RECARGA - (PREPAGO)"
        case 'MOVILNET_POSPAY':
          return "MOVILNET POSPAGO"
        case 'MOVISTAR_POSPAY':
          return 'MOVISTAR POSPAGO'
        case 'CANTV_POSPAY':
          return 'CANTV POSPAGO'
        case 'CANTV_INTERNET':
          return 'CANTV INTERNET - (POSPAGO)'
				default:{
					return null;
				}
  }
...
function downloadFile(url, name, ext){
  fetch(url)
		  .then(response => {
...
function keyPressValidarLetrasyOTROCaracteres(event) {
  var key = event.keyCode || event.which;
  var tecla = String.fromCharCode(key).toLowerCase();
